...
pub fn init_tracing(config: TracingConfig) {
    ...
    // Build env filter - respect RUST_LOG if set, otherwise use our level
    let make_filter = |level: Level| {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            // Default filter: show our crate at configured level, dependencies at WARN
    ...
    if let Some(log_path) = &config.log_file {
        ...
        let file = match std::fs::File::create(log_path) {
            ...
            Err(e) => {
                ...
                init_terminal_only(terminal_level, &config.format);
                let _ = TRACING_INITIALIZED.set(());
                return;
            }
        };

        let file_filter = EnvFilter::new("os402=debug,warn");

        // File layer - always DEBUG level, no ANSI colors
    ...
    if let Some(level) = terminal_level {
        let terminal_filter = make_filter(level);
...
/// Initialize terminal-only logging
fn init_terminal_only(level: Option<Level>, format: &LogFormat) {
