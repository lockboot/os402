...
/// * `verbose` - Whether to print verbose output about client configuration
...
pub fn x402_client(
    ...
    // Build base reqwest client with configuration (rustls provider installed in main)
    let mut reqwest_builder = reqwest::Client::builder().use_rustls_tls();
    ...
    if let Some(signer) = key.load_key()? {
        ...
        if verbose {
            println!("* x402 signer: {:?}", signer.address());
            println!("* x402 prefs: {}", payment_args.pay);
            if !max_limits.is_empty() {
                println!("* x402 limits:");
                for limit in &max_limits {
                    match limit {
                        MaxSpendLimit::Global { amount } => {
                            println!("*    Global: ${:.2}", amount);
                        }
                        MaxSpendLimit::PerToken { token, amount } => {
                            println!("*    {}: ${:.2}", token, amount);
                        }
                        MaxSpendLimit::PerTokenNetwork {
                            ...
                            println!("*    {}@{}: ${:.2}", token, network, amount);
                        }
                    }
                }
            }
