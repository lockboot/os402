...
impl TaskManager {
    ...
    pub fn detect_system_resources() -> Result<ResourceCapacity> {
    ...
    pub fn detect_system_usage() -> Result<SystemUsage> {
    ...
    pub async fn execute(
        ...
        // Increment resource usage (global)
        {
            ...
            usage.used_ram_mb += resources.ram_mb as u64;
        ...
        if let Some(ref pool_name_str) = pool_name {
            if let Some(ref pool_states) = self.pool_states {
                ...
                if let Some(pool_state) = pool_states_guard.get_mut(pool_name_str) {
                    pool_state.usage.used_ram_mb += resources.ram_mb as u64;
        ...
        let (stdin, args, env) = {
            ...
            let mut final_stdin = task.input.stdin.clone();
            ...
            // Merge private stdin if present (prepend to user stdin)
            if let Some(ref private_stdin_bytes) = task.secrets.stdin {
                if task.signed_offer.payload.stage2.stdin_private {
                    let private_stdin_str = String::from_utf8_lossy(private_stdin_bytes).to_string();
                    final_stdin = private_stdin_str + &final_stdin;
                }
            }

            (
                final_stdin.as_bytes().to_vec(),
        ...
        let output = {
            ...
            // Sign the output now that it's complete
            if let Err(e) = output_arc.sign(self.signer.as_ref()).await {
                eprintln!("Failed to sign task outputs: {}", e);
            }
            ...
            if result.killed_by_timeout {
                ...
                eprintln!("Task killed due to buffer overflow (stdout: {}, stderr: {})",
                    result.killed_by_stdout_overflow, result.killed_by_stderr_overflow);
        ...
        {
            let mut usage = self.usage.write().await;
            usage.used_ram_mb = usage.used_ram_mb.saturating_sub(resources.ram_mb as u64);
            usage.used_bogomips -= cpu_units as u64;
        }
        ...
        if let Some(ref pool_name_str) = pool_name {
            if let Some(ref pool_states) = self.pool_states {
                let mut pool_states_guard = pool_states.write().await;
                if let Some(pool_state) = pool_states_guard.get_mut(pool_name_str) {
                    pool_state.usage.used_ram_mb = pool_state.usage.used_ram_mb.saturating_sub(resources.ram_mb as u64);
