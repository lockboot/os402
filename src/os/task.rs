...
impl TaskOutput {
    ...
    /// Sign the stdout and stderr buffers with the provided signer
    /// This should be called when the task completes
    /// Can only be called once - subsequent calls will return an error
    pub async fn sign<S:Signer + ?Sized>(&self, signer: &S) -> anyhow::Result<()>
    {
        let stdout_sig = self.stdout.read().await.sign(signer).await?;
        let stderr_sig = self.stderr.read().await.sign(signer).await?;

        self.stdout_signature.set(format!("0x{}", hex::encode(stdout_sig.as_bytes())))
            .map_err(|_| anyhow::anyhow!("stdout_signature already set"))?;
        self.stderr_signature.set(format!("0x{}", hex::encode(stderr_sig.as_bytes())))
...
impl TaskInput {
    ...
    pub fn digest(&self) -> Vec<u8> {
        ...
        let args_hash = {
            ...
            sha256!(args_json.as_bytes()).finalize()
        };

        // Compute H(stdin)
        let stdin_hash = sha256!(self.stdin.as_bytes()).finalize();

        // Compute H(env)
        let env_hash = {
            let env_json = serde_json::to_string(&self.env).unwrap();
            sha256!(env_json.as_bytes()).finalize()
        };
...
impl Task {
    ...
    pub async fn completion_info(&self) -> Option<TaskCompletionInfo> {
        ...
        Some(TaskCompletionInfo {
            exit_code: output.exit_code.get().copied(),
            completed_at: output.completed_at.get().copied(),
            rusage: output.rusage.get().cloned(),
        })
    }
}
