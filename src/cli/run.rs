...
pub async fn run(args: RunArgs, config: &GlobalConfig) -> Result<()> {
    ...
    let client = x402_client(&payment_args, token_registry, &key_args, config.verbose, Some(config), None)?;
    ...
    for (i, (instance, offer_hash, offer)) in matching_offers.iter().enumerate() {
        ...
        println!(
            "    Resources: {} CPU, {} MB RAM",
            offer.limits.cpu_units, offer.limits.ram_mb
        );
