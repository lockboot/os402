...
pub async fn run(args: OfferSignArgs, config: &crate::config::GlobalConfig) -> Result<()> {
    ...
    let ram_mb = args.ram_mb
        .or_else(|| mcp_limits.and_then(|l| l.ram_mb))
        .unwrap_or(512);
    let buffer_capacity = args.buffer_capacity
        .unwrap_or(1024 * 1024);  // MCP schema doesn't have buffer_capacity
