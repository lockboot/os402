...
use crate::os::Task;
use super::super::{AppState, ErrorResponse, error_bad_request, error_not_found, error_internal};
...
pub async fn cgi_handler(
    ...
    if let Some(existing_task_arc) = existing {
        ...
        return build_cgi_response(task_id, &stdout_data);
    }
    ...
    state.with_payment(price_usd, offer_for_page, payment_request, move |_payer| async move {
        ...
        let (task_id, task_arc) = match state_clone.execute_task_from_offer(
            ...
            Ok(result) => result,
            Err(e) => return Ok(error_internal(&e.to_string())),
        };
        ...
        Ok(build_cgi_response(task_id, &stdout_data))
...
fn build_cgi_response(_task_id:String, stdout_data: &[u8]) -> Response {
    ...
    /*
    if let Ok(header_value) = axum::http::HeaderValue::from_str(&task_id.clone()) {
        response_headers.insert("X-OS402-TaskID", header_value);
    }
    */
